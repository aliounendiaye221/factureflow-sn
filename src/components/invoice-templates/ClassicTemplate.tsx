...
export default function ClassicTemplate({ invoice, agency, statusConfig, document_type = 'invoice', actions }: InvoiceTemplateProps) {
    ...
    return (
        <div className="p-6 print:p-0 [print-color-adjust:exact] [-webkit-print-color-adjust:exact] text-[13px]">
            ...
            {isInvoice && (
                <div className="mb-3 border border-gray-200 rounded-lg p-3 text-xs text-gray-600">
                    <div className="flex justify-between items-start gap-3">
                        <div className="space-y-0.5 flex-1">
                            <p className="font-semibold text-gray-800 mb-1">Modes de paiement :</p>
                            {agency?.wave_number && <p>Wave : <span className="font-mono font-semibold text-gray-900">{agency.wave_number}</span></p>}
                            {agency?.om_number && <p>Orange Money : <span className="font-mono font-semibold text-gray-900">{agency.om_number}</span></p>}
                            {agency?.bank_name && agency?.bank_iban && (
                                <p>Virement : <span className="font-semibold text-gray-900">{agency.bank_name}</span> — <span className="font-mono text-[11px]">{agency.bank_iban}</span></p>
                            )}
